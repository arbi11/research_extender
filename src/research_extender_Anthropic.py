...
class ResearchExtender:
    """Generate code and paper extensions using LLM assistance"""
    ...
    async def find_gaps(self, latex_data: Dict, code_data: Dict, mappings: List[Dict]) -> List[Dict]:
        ...
        for gap in structural_gaps:
            if gap['type'] == 'missing_implementation':
                ...
                gap['llm_suggestion'] = response.content[0].text
                ...
                gap['algorithm_design'] = response.content[0].text
                gap['related_context'] = context
...
async def main():
    ...
    latex_file = "../2509.13310v1.pdf"  # Should be .tex file
    code_paths = ["../DeepResearch-main/DeepResearch-main/inference/"]
    output_dir = "./research_extensions"

    # Build knowledge graph
    print("Building knowledge graph...")
    builder = KnowledgeGraphBuilder(working_dir="./research_graph")
